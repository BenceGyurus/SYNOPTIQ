...
export default function Home() {
  ...
  const string1Power = useMemo(() => ((latest?.vpv1 || 0) * (latest?.ipv1 || 0)) / 10, [latest]);
  const string2Power = useMemo(() => ((latest?.vpv2 || 0) * (latest?.ipv2 || 0)) / 10, [latest]);
          ...
          <div className="glass-advanced p-6 lg:p-12 relative overflow-hidden flex items-center justify-center min-h-[450px] lg:min-h-[550px]">
            ...
            <div className="relative w-full max-w-5xl">
              <svg viewBox="0 0 1000 500" className="w-full h-auto overflow-visible drop-shadow-[0_0_50px_rgba(0,0,0,0.5)]">
                ...
                {[
                  { id: 'ALPHA', y: 150, pwr: string1Power, v: (latest?.vpv1 || 0) / 10, i: latest?.ipv1, color: '#00f3ff' },
                  { id: 'BETA', y: 320, pwr: string2Power, v: (latest?.vpv2 || 0) / 10, i: latest?.ipv2, color: '#ff00ea' }
                ].map(s => (
